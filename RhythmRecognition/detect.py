from RhythmRecognition.initialize import *
...
def novelty_function(audiofile: str,
    ...
    """
    novelty = init_novelty(audiofile=audiofile,
                           approach=approach,
                           duration=duration,
                           gamma=gamma,
...
def tempo(audiofile: str,
          ...
          window_length: int = WIN_LENGTH) -> int:
    """Get dominant tempo of the given song.

    :param audiofile: Name of the input audio file.
    :param approach: Which tempogram to use - options are fourier or autocorrelation.
    ...
    :param frame_length: Number of samples in a frame.
    :param hop_length: Number of samples by which we have to advance between two consecutive frames.
    :param window_length: Length of the onset autocorrelation window (in frames). Used only for
    ...
    t = init_tempogram(approach=approach, novelty=novelty, similarity=similarity,
...
def tempogram(audiofile: str, approach: str = "fourier",
              ...
              hop_length: int = HOP_LENGTH,
              frame_length: int = FRAME_LENGTH,
              window_length: int = WIN_LENGTH) -> np.ndarray:
...
def beat_track(audiofile: str,
    ...
    :param tempo_approach: Which tempogram to use - options are fourier or autocorrelation.
    :param novelty_approach: Which novelty function to compute - options are spectral or energy.
    :param duration: Duration of the song in seconds. Only specify this parameter if you need to use a smaller
            part of the song. If not specified, it is set to the whole song duration.
    :param novelty_gamma: Compression factor for logarithmic compression used for computing the novelty function.
    ...
    beat_tracker = init_beat_tracker(approach, novelty, bpm, duration, tolerance_interval, alpha,
...
def beat_time_shift(audiofile: str,
                    ...
                    hop_length: int = HOP_LENGTH,
                    frame_length: int = FRAME_LENGTH,
                    window_length: int = WIN_LENGTH) -> float:
    ...
    beat_tracker = init_beat_tracker(approach=approach, novelty=novelty, tempo=bpm, duration=duration,
...
def rhythm_track(audiofile: str,
    ...
    rhythm_tracker = init_rhythm_tracker(approach=approach,
                                         audiofile=audiofile,
                                         novelty=novelty,
                                         duration=duration,
                                         bpm=bpm,
                                         ...
                                         alpha=alpha,
                                         num_rhythmic_onsets=num_rhythmic_onsets,
